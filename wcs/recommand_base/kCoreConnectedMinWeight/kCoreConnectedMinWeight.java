...
public class kCoreConnectedMinWeight {
    ...
    public static void main(String[] args) {
        ...
        try {
            BufferedReader br = new BufferedReader(new FileReader(datapath+"/data/"+ds+"/"+ds+"_newcore.txt"));// dblp min vertex = 0
                                                                                      // starts from 0
            try {
        ...
        // for (int i = 3; i < 4; i++) {
            ...
            try {
                ...
                BufferedReader br = new BufferedReader(new FileReader(datapath + "/data/"+ds+"/"+ds+"_queries_"+query_num+".txt"));
            ...
            for (int time = 0; time < arrli.size(); time++) {
                ...
                // find shortest path if node number more than 1
                if (newqueryNodes.length > 1) {
                    Integer qn = newqueryNodes[0];
                    while (qn != null) {
                        Vertex v = hmv.get(qn);
                    ...
                    splRes.pathNodeList.add(newqueryNodes[0]);

                }

                protectSet = run.protectNodes(queryCore, newqueryNodes, coreness, hm, hmv);
                
                resSet = new expandSet();

                if (splRes.pathNodeList.size() != 1) {
                    mst = splRes.pathNodeList;
                } else {
                    mst.add(newqueryNodes[0]);
                }
                ...
                do {
                    if (queryCore == 0){
                        ...
                        int resultCore = run.getMaxCore(cmap);
                        ArrayList<Integer> newvertexLargerCoreList = run.getVertexLargerCore(queryCore);
                        result = run.getSubGraph(newvertexLargerCoreList, cmap, newqueryNodes, queryCore, protectSet.expRes, removalN);
					    ...
					    if (negTag == 1){
                            ...
                            do {
                                if (queryCore == 0){
                                    result = new ArrayList<>(Arrays.asList(queryNodes));
                                    hSet.addAll(Arrays.asList(queryNodes));
                                    run.finalGraphWeightSum = run.getGraphWeightSum(result, hSet, hm);
                                    System.out.println("querycore is 0 but the node is still not added");
                                    break;
                                }
            ...
            try {
                ...
                BufferedWriter bw = new BufferedWriter(new FileWriter(datapath+"/data/"+ds+"/"+ds+"_res_gt_base.txt"));
